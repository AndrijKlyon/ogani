...
<section class="content-header">
    <h1>
        ...
        <a id="delete_all-button" data-model="brands" href="#" class="btn btn-sm btn-danger disabled">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-barcode"></i> Бренды</li>
    </ol>
</section>
<!-- Main content -->
                ...
                <div class="box box-primary">
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach($brands as $item)
                                        <tr data-id="{{ $item['id'] }}" data-model="Brand">
                                            <td>{{ $item['id'] }}</td>
                                            ...
                                            <td class="align-middle">
                                                <form action="{{ route('brands.destroy', ['brand' => $item['id']]) }}" method="POST">
                                                    ...
                                                    <a class="btn btn-social-icon btn-vk"
                                                        href="{{ route('brands.show', ['brand'=> $item['id']]) }}"
                                                        data-toggle="tooltip"
                                                        title="Просмотреть"><i class="fa fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-social-icon btn-instagram"
                                                        href="{{ route('brands.edit', ['brand'=> $item['id']]) }}"
                                                        data-toggle="tooltip"
                    ...
                    @if($brands->hasPages() )
                        <div class="box-footer text-center">
                            {{ $brands->links('admin.paginate-table_items') }}
                        </div>
                    @endif
                </div>
            </div>
