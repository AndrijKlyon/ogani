...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-user"></i> Редактирование пользователя</li>
    </ol>
</section>
<!-- Main content -->
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-primary">
                <form action="{{ route('users.update', ['user' => $user['id']]) }}" method="post" data-toggle="validator" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group has-feedback">
                            ...
                            <select name="role" class="form-control" required>
                                <option value="admin" {{ ($user['role'] == 'admin' || old('role') == 'admin')  ? 'selected': '' }}>Администратор</option>
                                <option value="user" {{ ($user['role'] == 'user' || old('role') == 'user') ? 'selected': '' }}>Пользователь</option>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Аватар</label>
                            <input name="file" id="file" type='file' value="" onchange="readURL(this);" ><br>
                            @if($user->img && $user->img != null && Storage::disk('local_public')->exists($user->img))
                                <img style="width: 100px; border-radius: 50%" class="p-2 blah" src="{{ asset($user->img) }}" alt="{{ $user->name }}" />
                            @else
                                <img class="p-2 blah" src="{{ Croppa::url('img/no-image.png', 70, 70) }}" alt="{{ $user->name }}">
                            @endif
