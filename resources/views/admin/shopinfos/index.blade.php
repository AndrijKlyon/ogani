...
<section class="content-header">
    <h1>
        <a href="{{ route('shopinfos.create') }}" class="btn btn-sm btn-success">
            <i class="fa fa-plus"></i> Добавить пост
        </a>
        <a id="delete_all-button" data-model="shopinfos" href="#" class="btn btn-sm btn-danger disabled">
    ...
    <ol class="breadcrumb">
        ...
        <li class="active"><i class="fa fa-file-text"></i> Справочные статьи</li>
    </ol>
</section>
<!-- Main content -->
            ...
            <div class="box box-primary">
                <div class="box-body box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Справочные статьи</h3>
                    </div>
                    <div class="box-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach($shopinfos as $item)
                                        <tr data-id="{{ $item['id'] }}" data-model="Shopinfo">
                                            <td>{{ $item['id'] }}</td>
                                            ...
                                            <td>
                                                <form action="{{ route('shopinfos.destroy', ['shopinfo'=> $item['id']]) }}" method="POST">
                                                    @csrf
                                                    <a class="btn btn-social-icon btn-vk"
                                                        href="{{ route('shopinfos.show', ['shopinfo'=> $item['id']]) }}"
                                                        data-toggle="tooltip"
                                                        title="Просмотреть"><i class="fa fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-social-icon btn-instagram"
                                                        href="{{ route('shopinfos.edit', ['shopinfo'=> $item['id']]) }}"
                                                        data-toggle="tooltip"
                    ...
                    @if( $shopinfos->hasPages() )
                        <div class="box-footer text-center">
                            {{ $shopinfos->links('admin.paginate-table_items') }}
                        </div>
                    @endif
                </div>
            </div>
